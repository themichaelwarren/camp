  ...
  const renderSidebarContent = (collapsed: boolean, isMobile = false) => (
    <>
      ...
      <nav className={`${isMobile ? '' : 'flex-1'} overflow-y-auto ${collapsed ? 'px-2' : 'px-4'} py-4`}>
        {isMobile ? (
          /* Mobile: 2-column grid with section headers */
          <div>
            {menuGroups.map((group, gi) => (
              <div key={group.label}>
                <p className={`text-[10px] font-bold text-indigo-400/60 uppercase tracking-widest px-1 ${gi === 0 ? 'mb-1' : 'mt-3 mb-1'}`}>
                  {group.label}
                </p>
            ...
            {isLoggedIn && (
              <div>
                <p className="text-[10px] font-bold text-indigo-400/60 uppercase tracking-widest px-1 mt-3 mb-1">More</p>
                <div className="grid grid-cols-2 gap-1">
                  {[
            ...
            collapsed ? (
              ...
              <div className={`rounded-xl p-3 cursor-pointer transition-colors group/mini ${isJukeboxMode ? 'bg-indigo-950/70 border border-amber-500/40 shadow-[0_0_12px_rgba(245,158,11,0.15)]' : 'bg-indigo-950/70 border border-indigo-800 hover:border-indigo-600'}`} onClick={() => { setShowNowPlaying(true); setIsMobileNavOpen(false); }}>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg text-indigo-600 flex items-center justify-center overflow-hidden flex-shrink-0 relative">
                    <ArtworkImage
                      fileId={player.artworkFileId}
                      fallbackUrl={player.artworkUrl}
                      alt={`${player.title} artwork`}
                    ...
                    />
                    <div className="absolute inset-0 bg-indigo-950 flex items-center justify-center opacity-0 group-hover/mini:opacity-100 transition-opacity">
                      <i className="fa-solid fa-chevron-up text-indigo-200 text-sm"></i>
                    </div>
                  </div>
        ...
        {isLoggedIn ? (
          isMobile ? (
            ...
            <div className="space-y-3">
              ...
              {/* User row with inline theme + logout */}
              <div className="flex items-center gap-3">
                ...
                {onThemeChange && (
                  <div className="flex items-center gap-0.5">
                    {([
                      { mode: 'light' as const, icon: 'fa-sun' },
                      { mode: 'dark' as const, icon: 'fa-moon' },
                      { mode: 'system' as const, icon: 'fa-desktop' },
                    ]).map(({ mode, icon }) => (
                      <button
                        key={mode}
                        onClick={() => onThemeChange(mode)}
                        className={`theme-picker-btn w-7 h-7 rounded-full flex items-center justify-center transition-colors ${
                ...
                <button
                  onClick={onLogout}
                  className="w-7 h-7 rounded-full bg-white/10 hover:bg-rose-500/20 text-indigo-200 hover:text-rose-200 flex items-center justify-center transition-colors"
                  aria-label="Log out"
                >
